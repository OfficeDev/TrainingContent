# Demo: Create and add Office 365 Connectors to teams

This completed project is the result of the lab exercise **Create and add Office 365 Connectors to teams** that is referenced in the [README](./../../README.md) in this repo.
...
- Follow the instructions in the associated lab exercise to register a Connector and copy the Connector ID.
- Rename the file **.env.example** to **.env**.
  - Set the connector ID to the ID of the connector you just registered.
  - You do not need to edit any other values in this file unless you have an existing ngrok license with a reserved subdomain name & auth key. These are only available to paid paid ngrok accounts, but it is not necessary to run the demo.
- Follow the instruction sin the associated lab exercise, specifically the section **Test the Office 365 Connector in Microsoft Teams**, to run the demo.